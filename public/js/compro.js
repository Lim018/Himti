...
function initHeroAnimations() {
  ...
  const heroRightChildren = document.querySelectorAll(".hero .right > *"); // All direct children of .hero .right

  // Animate hero elements on load
  setTimeout(() => {
    // Animate heroLeft container (from left)
    if (heroLeft) {
      heroLeft.style.opacity = "0";
      heroLeft.style.transform = "translateX(-50px)";
      heroLeft.style.transition = "all 1.5s cubic-bezier(0.175, 0.885, 0.32, 1.275)";
    ...
    // Animate heroRight container (from right)
    if (heroRight) {
      heroRight.style.opacity = "0";
      heroRight.style.transform = "translateX(50px)";
      heroRight.style.transition = "all 1.5s cubic-bezier(0.175, 0.885, 0.32, 1.275)";
    ...
    // Animate heroTitle (from left)
    if (heroTitle) {
      heroTitle.style.opacity = "0";
      heroTitle.style.transform = "translateX(-50px)";
      heroTitle.style.transition = "all 1.5s cubic-bezier(0.175, 0.885, 0.32, 1.275)";
    ...
    // Animate heroSubtitle (from left)
    if (heroSubtitle) {
      heroSubtitle.style.opacity = "0";
      heroSubtitle.style.transform = "translateX(-30px)";
      heroSubtitle.style.transition = "all 1.5s cubic-bezier(0.175, 0.885, 0.32, 1.275)";
    ...
    // Animate heroButton (from left with scale)
    if (heroButton) {
      heroButton.style.opacity = "0";
      heroButton.style.transform = "translateX(-20px) scale(0.9)";
      heroButton.style.transition = "all 1.2s cubic-bezier(0.175, 0.885, 0.32, 1.275)";

      setTimeout(() => {
        heroButton.style.opacity = "1";
        heroButton.style.transform = "translateX(0) scale(1)";
      }, 1050);
    }

    // Animate all direct children of heroRight (from right)
    heroRightChildren.forEach((child, index) => {
      child.style.opacity = "0";
      child.style.transform = "translateX(30px)";
      child.style.transition = `all 1.2s cubic-bezier(0.175, 0.885, 0.32, 1.275) ${0.6 + index * 0.3}s`;
    ...
    // Clear transitions after entrance animations to avoid conflicts with parallax/floating
    setTimeout(() => {
      if (heroLeft) heroLeft.style.transition = "";
      if (heroRight) heroRight.style.transition = "";
      heroRightChildren.forEach((child) => {
        child.style.transition = "";
      });
    }, 3000); // Matches longest animation (1350 + max index * 300)
  }, 100);
...
// Modified parallax and floating animations
function initParallaxEffects() {
  ...
  if (heroLeft && heroRight) {
    ...
    const animate = (time) => {
      // Skip first 3 seconds to allow entrance animations to complete
      if (time < 3000) {
